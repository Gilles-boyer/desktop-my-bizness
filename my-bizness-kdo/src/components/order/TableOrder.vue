...
export default {
  ...
  methods: {
    ...
    ...mapActions(["deleteOrder", "initOrders"]),
    displayPdfVoucher(data) {
      fetch(`https://mybizness.herokuapp.com/api/voucher/display/pdf/${data.voucher.num}`, {/*YOUR CUSTOM HEADER*/} ) // FETCH BLOB FROM IT
    ...
    downloadPdfvoucher(data) {
      window.open(`https://mybizness.herokuapp.com/api/voucher/download/pdf/${data.voucher.num}`, "_blank");
    },
    getDateFormat(date) {
      if (date) {
